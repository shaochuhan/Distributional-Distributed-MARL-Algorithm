...
class CACCEnv:
    ...
    def _get_state(self):
        state = []
        # 计算所有智能体中的最大邻居数
        neighbor_counts = [np.sum(self.neighbor_mask[i] == 1) for i in range(self.n_agent)]
        max_neighbors = max(neighbor_counts)

        for i in range(self.n_agent):
            cur_state = [self._get_veh_state(i)]
            neighbors = np.where(self.neighbor_mask[i] == 1)[0]
            if self.agent.startswith('ia2c') or self.agent.startswith('wpo'):
                for j in np.where(self.neighbor_mask[i] == 1)[0]:
                    cur_state.append(self._get_veh_state(j))
            if self.agent.startswith('d2ac'):
                for j in neighbors:
                    cur_state.append(self._get_veh_state(j))       
                # 填充到最大邻居数，确保所有智能体状态维度一致
                
                # current_neighbors = len(neighbors)
                # while current_neighbors < max_neighbors:
                #     # 用零状态填充缺失的邻居
                #     zero_state = np.zeros_like(self._get_veh_state(0))
                #     cur_state.append(zero_state)
                #     current_neighbors += 1

                # 添加邻居状态并填充
                # for j in range(max_neighbors):
                #     if j < len(neighbors):
                #         cur_state.append(self._get_veh_state(neighbors[j]))
                #     else:
                #         cur_state.append(np.zeros_like(self._get_veh_state(i)))
            if self.agent == 'ia2c_fp':
                ...
                # for j in range(max_neighbors):
                #     if j < len(neighbors):
                #         cur_state.append(self.fp[neighbors[j]])
                #     else:
                #         cur_state.append(np.zeros(self.fp.shape[1]))
    ...
    def _init_space(self):
        ...
        for i in range(self.n_agent):
            ...
            if i >= 1:
                self.neighbor_mask[i,i-1] = 1  # 前一个邻居
            # if i >= 2:
            #     self.neighbor_mask[i,i-2] = 1
            if i <= self.n_agent-2:
                self.neighbor_mask[i,i+1] = 1  # 后一个邻居
            # if i <= self.n_agent-3:
            #     self.neighbor_mask[i,i+2] = 1
